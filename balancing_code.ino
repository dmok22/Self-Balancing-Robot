...
enum SonarState { IDLE, TO_CENTER, TO_LEFT, READ_LEFT, TO_RIGHT, READ_RIGHT, RETURN_CENTER, DONE };
...
void loop() {
  handleBLECommands();
  combine();
  handleSonarSweep();  // ✅ NEW: non-blocking sonar FSM
  PID(angle);
...
float getSonarDistance() {
  ...
  return duration == 0 ? -1 : distance_cm;
}
